...
class Uploader {
    ...
    public function __construct($files, $upload_path = NULL) {
        $this->files = $files;
        if ($upload_path) {
    ...
    public function doUpload($fileField, $allowType = NULL) {
        $return = TRUE;
        if ($allowType) {
            $this->allowType = $allowType === '*' ? [] : $this->allowType;
        }
        /* 检查文件路径是滞合法 */
        ...
        /* 将文件上传的信息取出赋给变量 */
        $file = isset($this->files[$fileField]) ? $this->files[$fileField] : [];
        if (!$file) {
            return FALSE;
        }
        $name = $file['name'];
    ...
    public function getError() {
        ...
        case 2:
            ...
            break;
        case 1:
            $str .= "上传的文件超过了upload_max_filesize选项限制的值";
            break;
        case -1:
            $str .= "未允许类型";
            break;
